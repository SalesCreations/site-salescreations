/* eslint-disable vue/component-definition-name-casing */
import Vue from 'vue'
import Page from '~/components/Page.vue'
import ListSkills from '~/components/lists/ListSkills.vue'
import ItemList from '~/components/lists/ItemList.vue'
import BasicSection from '~/components/sections/BasicSection.vue'
import Timeline from '~/components/timeline/Timeline.vue'
import IframeSpotify from '~/components/shared/IframeSpotify.vue'

Vue.component('Page', Page)
Vue.component('BasicSection', BasicSection)
Vue.component('ListSkills', ListSkills)
Vue.component('ItemList', ItemList)
Vue.component('Timeline', Timeline)
Vue.component('IframeSpotify', IframeSpotify)
